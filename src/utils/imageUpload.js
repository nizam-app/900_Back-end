...
const IMAGE_UPLOAD_SERVICE_URL = process.env.IMAGE_UPLOAD_SERVICE_URL || 'https://img.mtscorporate.com';
...
export const uploadImageToService = async (file) => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.error || `Image upload failed: ${response.status}`);
    }
