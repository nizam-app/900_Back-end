...
export const getNotifications = async (req, res, next) => {
  try {
    ...
    if (notifications.length > 0 && notifications[0].user) {
      console.log(
        `📱 User ${userId} has ${notifications[0].user.fcmTokens?.length || 0} active FCM token(s)`
      );
    }
...
export const registerFCMToken = async (req, res, next) => {
  try {
    ...
    if (!isValidFCMToken(fcmToken)) {
      console.warn(
        `⚠️ Invalid FCM token format rejected for user ${userId}: ${fcmToken.substring(0, 30)}...`
      );
      return res.status(400).json({
        message: "Invalid FCM token format. Please provide a valid FCM token from your device.",
      });
    }
    ...
    if (existingToken) {
      ...
      console.log(
        ...
        }) - Token ID: ${updatedToken.id}, Previous User: ${existingToken.userId}`
      );
    } else {
  ...
  try {
    ...
    for (const tokenRecord of allTokens) {
      if (!isValidFCMToken(tokenRecord.token)) {
        ...
        console.log(
          `🗑️ Invalid token format found: ${tokenRecord.id} (user ${tokenRecord.userId}, device: ${tokenRecord.deviceType || "unknown"})`
        );
      }
    }
