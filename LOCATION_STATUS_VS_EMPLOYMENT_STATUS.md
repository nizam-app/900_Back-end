  ...
  return (
    <div className='technician-card'>
      ...
      <div className='employment-status'>
        <span
          className={`badge ${technician.technicianProfile.status.toLowerCase()}`}
        >
...
function DispatcherAssignmentView({ technicians }) {
  ...
  const onlineReady = technicians.filter(
    (tech) =>
      tech.technicianProfile.status === "ACTIVE" &&
      tech.locationStatus === "ONLINE"
  );

  const busy = technicians.filter((tech) => tech.locationStatus === "BUSY");
  ...
  const inactive = technicians.filter(
    (tech) => tech.technicianProfile.status !== "ACTIVE"
  );
...
function StatusBadge({ locationStatus, employmentStatus }) {
  ...
  return <span className={`status-badge bg-${color}`}>{text}</span>;
}
```
...
function TechnicianDashboard({ technicians }) {
  const stats = {
    totalActive: technicians.filter(
      (t) => t.technicianProfile.status === "ACTIVE"
    ).length,

    availableNow: technicians.filter(
      (t) =>
        t.technicianProfile.status === "ACTIVE" && t.locationStatus === "ONLINE"
    ).length,

    onJobs: technicians.filter((t) => t.locationStatus === "BUSY").length,
    ...
    inactive: technicians.filter((t) => t.technicianProfile.status !== "ACTIVE")
      .length,
  };
...
function ActionButtons({ tech }) {
  ...
  return (
    <>
      <button disabled={!canAssign} onClick={() => assignJob(tech.id)}>
...
const showOnMap =
  status === "ACTIVE" && // Is employed
  lastLatitude &&
  lastLongitude; // Has GPS data

// Show as "available"?
const available = status === "ACTIVE" && locationStatus === "ONLINE";
```
