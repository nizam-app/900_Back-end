...
const testEndpoint = async () => {
  try {
    ...
    if (serviceRequests.length === 0) {
      ...
      console.log(
        "   💡 Create a Service Request first before converting to WO"
      );
      ...
      if (allSRs.length > 0) {
        ...
        allSRs.forEach((sr) => {
          console.log(
            `   - SR ID: ${sr.id}, Number: ${sr.srNumber}, Status: ${sr.status}`
          );
        });
      }
    ...
    console.log(
      `   ✅ Found ${serviceRequests.length} available Service Request(s):\n`
    );
    ...
    if (technicians.length === 0) {
      ...
      console.log(
        `   ✅ Found ${technicians.length} available technician(s):\n`
      );
    ...
    if (dispatchers.length === 0) {
      ...
      console.log(
        "   💡 You need a DISPATCHER or ADMIN role to convert SR to WO"
      );
    } else {
      console.log(
        `   ✅ Found ${dispatchers.length} dispatcher(s)/admin(s):\n`
      );
    ...
    if (serviceRequests.length > 0 && dispatchers.length > 0) {
      ...
      console.log("\nHeaders:");
      console.log(
        `Authorization: Bearer <token-for-user-id-${exampleDisp.id}>`
      );
      ...
      console.log(
        `   2. Login as dispatcher/admin (User ID: ${exampleDisp.id})`
      );
      if (exampleTech) {
