...
async function checkCalculation() {
  try {
    ...
    console.log(`   Freelancer Commission Rate: ${config.freelancerCommissionRate} (${config.freelancerCommissionRate * 100}%)`);
    console.log(`   Internal Bonus Rate: ${config.internalEmployeeBonusRate} (${config.internalEmployeeBonusRate * 100}%)`);
    ...
    console.log(`   Commission = Payment × ${config.freelancerCommissionRate} (${config.freelancerCommissionRate * 100}%)`);
    console.log("");
    console.log("   For INTERNAL technicians:");
    console.log(`   Bonus = Payment × ${config.internalEmployeeBonusRate} (${config.internalEmployeeBonusRate * 100}%)`);
    ...
    console.log("\n   FREELANCER (Commission at " + (config.freelancerCommissionRate * 100) + "%):");
    examplePayments.forEach(amount => {
      const commission = amount * config.freelancerCommissionRate;
      console.log(`   ₹${amount.toLocaleString()} → Commission: ₹${commission.toLocaleString()}`);
    });
    
    console.log("\n   INTERNAL EMPLOYEE (Bonus at " + (config.internalEmployeeBonusRate * 100) + "%):");
    examplePayments.forEach(amount => {
      const bonus = amount * config.internalEmployeeBonusRate;
      console.log(`   ₹${amount.toLocaleString()} → Bonus: ₹${bonus.toLocaleString()}`);
    });
    ...
    if (recentCommissions.length === 0) {
      ...
      recentCommissions.forEach((c, idx) => {
        ...
        console.log(`      Technician: ${c.technician?.name} (${c.technician?.technicianProfile?.type || c.technician?.role})`);
    ...
    console.log(`   Total Amount: ₹${stats._sum.amount?.toLocaleString() || 0}`);
    
    byType.forEach(t => {
      console.log(`   ${t.type}: ${t._count.id} records, ₹${t._sum.amount?.toLocaleString() || 0}`);
    });
